...
<div class="users-page mx-auto w-full max-w-6xl">
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <h2 class="text-xl font-semibold text-slate-900"><?= htmlspecialchars(__('users.title')) ?></h2>
        ...
        <div class="-mx-4 overflow-x-auto sm:mx-0">
            <table class="min-w-full divide-y divide-slate-200 text-sm">
                <thead class="bg-slate-50 text-left text-xs font-semibold uppercase tracking-wide text-slate-600">
                    <tr>
                        <th class="px-4 py-3"><?= htmlspecialchars(__('users.name')) ?></th>
                        <th class="px-4 py-3">Email</th>
                        <th class="hidden px-4 py-3 sm:table-cell"><?= htmlspecialchars(__('users.phone')) ?></th>
                        <th class="px-4 py-3"><?= htmlspecialchars(__('users.locale')) ?></th>
                        <th class="px-4 py-3"><?= htmlspecialchars(__('users.role')) ?></th>
                        <th class="px-4 py-3"><?= htmlspecialchars(__('users.status')) ?></th>
                        <th class="w-28 px-4 py-3 text-right"><?= htmlspecialchars(__('users.actions')) ?></th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100" id="users-table-body">
                    <?php foreach ($users as $user): ?>
                        ...
                        $roleLabel = match ($user['role']) {
                            ...
                            'gestor' => __('user.role_gestor'),
                            'vendedor' => __('user.role_seller'),
                            default => $user['role'],
                        };
                        $loc = strtoupper(htmlspecialchars($user['locale'] ?? 'es'));
                        $statusClass = $user['status'] === 'active' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600';
                        $statusLabel = $user['status'] === 'active' ? __('user.status_active') : __('user.status_inactive');
                        ?>
                        <tr class="hover:bg-slate-50/80" data-user-id="<?= $user['id'] ?>">
                            ...
                            <td class="px-4 py-3 text-slate-600"><?= $loc ?></td>
                            <td class="px-4 py-3"><span class="inline-flex rounded-md px-2 py-0.5 text-xs font-medium <?= $roleClass ?>"><?= htmlspecialchars($roleLabel) ?></span></td>
                            <td class="px-4 py-3"><span class="inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-xs font-medium <?= $statusClass ?>"><span class="h-1.5 w-1.5 rounded-full bg-current"></span><?= htmlspecialchars($statusLabel) ?></span></td>
                    ...
                    <?php if (empty($users)): ?>
                        <tr>
                            <td colspan="7" class="px-4 py-8 text-center text-slate-500"><?= htmlspecialchars(__('users.empty')) ?></td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
...
<div class="fixed inset-0 z-[400] hidden items-center justify-center bg-black/50 p-4" id="user-modal" aria-hidden="true">
    <div class="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-xl bg-white p-6 shadow-xl" role="document" onclick="event.stopPropagation()">
        <div class="mb-4 flex items-center justify-between">
            <h3 class="text-lg font-semibold text-slate-900" id="modal-title"><?= htmlspecialchars(__('users.new')) ?></h3>
        ...
        <form id="user-form" class="space-y-4">
            ...
            <div>
                ...
                <p class="mt-1 hidden text-xs text-slate-500" id="password-hint">min. 8</p>
            </div>
            <div>
            ...
            <div>
                <label class="mb-1 block text-sm font-medium text-slate-700" for="user-locale"><?= htmlspecialchars(__('users.locale')) ?></label>
                <select class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" id="user-locale" name="locale">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700" for="user-role"><?= htmlspecialchars(__('users.role')) ?> <span class="text-red-500">*</span></label>
                    <select class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20" id="user-role" name="role" required>
                        <option value="">—</option>
                        <option value="admin"><?= htmlspecialchars(__('user.role_admin')) ?></option>
                        <option value="gestor"><?= htmlspecialchars(__('user.role_gestor')) ?></option>
                        <option value="vendedor"><?= htmlspecialchars(__('user.role_seller')) ?></option>
                    </select>
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700" for="user-status"><?= htmlspecialchars(__('users.status')) ?></label>
                    <select class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20" id="user-status" name="status">
                        <option value="active"><?= htmlspecialchars(__('user.status_active')) ?></option>
                        <option value="inactive"><?= htmlspecialchars(__('user.status_inactive')) ?></option>
                    </select>
                </div>
            </div>
